...
export function toolDefinitions(hasBirth) {
  return [
    tool(
      "search_reference_cards",
      "需要情绪支持方法、关系建议或传统术语出处时，检索经过人工整理的短卡片。query 使用简短中文主题词。命理只在用户开启生辰参考时可查。",
      schema(
        {
...
export function searchCards(query, domain) {
  const q = terms(query);
  return cards
...
export function executeTool(call, birth) {
  try {
    if (call.function?.name === "search_reference_cards") {
      ...
      if (
        ...
        throw Error();
      const found = searchCards(query, domain);
      return {
        data: {
          cards: found,
...
export async function runAgent({
  ...
  const tools = toolDefinitions(!!chat.birth);
  ...
  if (Array.isArray(calls) && calls.length) {
    ...
    for (const call of calls) {
      const output = executeTool(call, chat.birth);
