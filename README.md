...
[产品意图与后续阶段](docs/PRODUCT.md) · [资料与技能筛选](docs/RESEARCH.md) · [回信风格与 rubric](docs/REPLY-RUBRIC.md) · [视觉素材记录](docs/ART.md)

## 已实现

- 每次进入先显示登录/注册或游客选择；关闭窗口也可继续。随后展示项目目的、玩法、隐私与 AI 使用边界公告，实际可见五秒后收起，可暂停阅读或从右下角指南重看。已登录用户可以直接继续；首次访问在公告后选择猫咪毛色。公告不会代替聊天数据处理同意。
