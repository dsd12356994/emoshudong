...
// Usage: node scripts/prepare-art.mjs <garden.png> <transparent-cat.png>
...
try {
  ...
  for (const [index, name] of ["garden", "cat"].entries()) {
    const source =
    ...
    const output = await page.evaluate(
      async ({ source, name }) => {
        ...
        c.width = name === "cat" ? 192 : img.width;
        c.height = name === "cat" ? 192 : img.height;
        c.getContext("2d").drawImage(img, 0, 0, c.width, c.height);
        return c
          .toDataURL("image/webp", name === "cat" ? 1 : 0.88)
          .split(",")[1];
      },
      { source, name },
    );
